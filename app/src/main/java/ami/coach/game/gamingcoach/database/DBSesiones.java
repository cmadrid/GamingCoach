...
public class DBSesiones {
    ...
    public Cursor consultar(String juego){
        ...
        String[] campos = new String[] {JUEGO, DBJuego.NOMBRE , MINUTOS, DBJuego.LOGO,ID,INICIO};
        //Cursor c = db.query(NOMBRE_TABLA, campos, "usuario=?(where)", args(para el where), group by, having, order by, num);
    ...
    public Cursor consultarAnio(String id){
        ...
        return db.query(NOMBRE_TABLA, campos, JUEGO+"="+id+" and "+INICIO+">datetime('now','-5 hours','-12 months')", null,"1,2", null, "1,2");
    }
